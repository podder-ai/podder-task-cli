    ...
    def _check_exisiting_process(self, process_name: str,
        ...
        if process_directory.exists():
            ...
            message = 'Process [{}] already exists. Do yo want to overwrite?'.format(
                process_name)
            if entity:
                if entity.base_repository != self._repository.url:
                    message = 'Process [{}] already exists but came from different repository. Do yo want to overwrite?'.format(
                        process_name)
