...
class ImportLibrary(ImportBase):
    ...
    def _copy_config(self):
        config = self._source.get_config()
        if config is None:
            click.secho("This library has no default config.", fg="yellow")
            return

        config_path = self._base_path.joinpath("config", self._source.name)
    ...
    def _create_process(self):
        process_path = self._base_path.joinpath("processes", self._source.name)
    ...
    def _update_process_file(self):
        process_file_path = self._base_path.joinpath("processes",
                                                     self._source.name,
                                                     "process.py")
        _object = self._source.get_entry("object")
        ...
        data = {
            ...
            "library_method": self._source.get_entry("method"),
    ...
    def _build_input(self) -> Tuple[str, str]:
        raw_inputs = self._source.get_entry("input")
    ...
    def _build_output(self) -> Tuple[str, str]:
        raw_outputs = self._source.get_entry("output")
        outputs = []
        build_payload = []
